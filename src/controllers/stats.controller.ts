...
export const getBarCharts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
    ...
    const transformedProducts = products.map(transformToMyDocument);
    const transformedUsers = users.map(transformToMyDocument);
    const transformedOrders = orders.map(transformToMyDocument);

    const productsCounts = getChartData({
      length: 6,
      docArr: transformedProducts,
      today,
    });
    const usersCounts = getChartData({
      length: 6,
      docArr: transformedUsers,
      today,
    });
